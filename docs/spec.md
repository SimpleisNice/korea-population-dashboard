...
> 최종 업데이트: 2026-08-19 (N-5 월간 리포트 · N-2 세대 분화)
> 대상: `main` 브랜치 기준
...
| 정렬 5종 | 총인구 / 전월 증감 / 전년 증감률 / 세대수 / **세대 분화** |
| 지표 설명 | 세대 분화 정렬일 때만 정의 안내 노출 |
...
**세대 분화(N-2)** = 세대수 증가율 − 인구 증가율(전년 대비, %포인트). 인구·세대 두 시계열을 조합한 값으로 KOSIS 에 없는 고유 지표다. 값이 클수록 한 가구에 사는 사람 수가 빠르게 줄고 있다는 뜻이다.

지역 상세의 서술(`buildRegionNarrative`)과 **같은 정의**를 쓴다 — 서술은 "이 지역이 어떤가"를, 순위는 "어디가 그런가"를 답한다. 새 화면·새 카드를 만들지 않고 기존 순위표에 정렬 하나를 더한 것이다(`principles.md` 0-1).

2026.07 분포: 최소 -6.40 · 25% 0.90 · 중앙 1.19 · 75% 1.47 · 최대 2.16%p. 226곳 중 음수 14곳. 전년 데이터가 없는 신설 지역은 `null`이며 목록 끝으로 정렬된다.
...
`npm run test` = `build-data` + `vitest run`. 총 **87개, 전부 통과**.

| 파일 | 개수 | 지키는 것 |
|---|---:|---|
| `src/lib/data.test.ts` | 31 | 빌드 산출물 회귀 — 계층 판별, 전국 합계 대조, 순위 계층 분리, 폴백 금지, 개편 시계열 연속성, 구별 현황 합산, **폐지 지역 처리**, **지도 시도명 일치** |
