...
export function RegionSearch({ regions }: Props) {
  ...
  return (
    <div className="relative">
      <div
        className="flex items-center gap-2 rounded-xl border px-4"
        style={{
      ...
      {open && results.length > 0 && (
        ...
        >
          {results.map(region => (
            <li key={region.code}>
              <button
                className="flex w-full items-center gap-2 border-b px-4 text-left text-sm transition-colors last:border-b-0 hover:bg-[var(--color-surface)]"
                style={{ height: 48, borderColor: '#F3F4F6' }}
                onMouseDown={() => handleSelect(region)}
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--color-text-secondary)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
